# iOS Demo
## Create an iOS app within 20 minutes
...
- Open Xcode
	- Select `Create a new Xcode project`
		...
		- Click `Next`
		- Click `Finish`
		- In window that pops up, click `New Folder` and name it `iOS Demo`
		- Click `Create`
- Testing your project
	- Start by running your project to test everything is working
	- 🎬  You can do that by click the ▶️ at the top of the window for Xcode, or by clicking `⌘ + r` on your keyboard
...
### Create the app components (3 minutes)

- On the right hand side, near the tool bar, click the `Main.storyboard` file
...
- Click the 🔘 icon which looks like an old iPhone 5 home button at the right-most area of the screen
	...
	- A menu should open
		...
		- type in `botton` and drag `Button` to the blank iPhone canvas
...
### Create the app logic (5 minutes)
- Click on the double-circle icon at the top right-most part of the screen near where the home button icon was before
	...
	- Now click on `Text Field` in the toolbar to the left and click `control` while dragging your mouse to line 19 in the window next to your iPhone canvas
		...
		- Name it `text_filed` and make sure `Outlet` is selected instead of `Action` just above the name field
		- Do the same ting for the `Text View` component but name it `text-view`
		- Do the same ting again for the `Button` component but name it `button` and make sure to select `Action` this time instead of `Outlet`
	- 👁 In the window to the right of your iPhone canvas you should now see two IBOutlets, 1 for your `text_view` and 1 for your `text_field`, follow by an IBAction for your button
	...
	- Enter the below line of code in between the curl braces `{}` for the IBAction component reference to your button:
		- `text_view.text = text_field.text`
		- 🎬 run your app to test your code
		- Refernce, screenshot <img width="1920" alt="8" src="https://user-images.githubusercontent.com/17191728/58447629-d9a0a600-80b9-11e9-8a3c-14127406e53d.png">
	- 🧪 Let's test the code we have written:
		- Enter your name in the `text_field` component and click the button next to it
		- 👁 you should now see the name you the `text_field` appear in the `text_view` component, yay!
			- Reference, screenshot
...
### Concluding thoughts (1 minutes)

Creating an iOS app is fun and easy! My hope is that you were able to create your own iOS app today and that you enjoyed doing so.
